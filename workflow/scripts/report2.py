...
def _add_overview_section(genotype_table, target_genes):
    ...
    for i, gene in enumerate(target_genes, 1):
        ...
        table += (
            f"      <tr style='text-align:center' class={color} media='all'>\n"
            f"          <th scope='row'>{i}</th>\n"
            f"          <td>{gene.upper()}</td>\n"
            f"          <td>{genotype}</td>\n"
            f"          <td>{phenotype}</td>\n"
...
def _add_recommendations_section(phenotype_table, drug_table):
    table = (
        ...
        "       <tr>\n"
        "           <th scope='col' style='width: 20%; media='all''>Drug</th>\n"
        "           <th scope='col' style='width: 80%; media='all''>Recommendation</th>\n"
        "       </tr>\n"
        "   </thead>\n"
        "   <tbody>\n"
    )
    ...
    if table == (
        ...
        return(
            "<div style='text-align:center media='all'' class='alert alert-success' role='alert'>\n"
            "<h4 class='alert-heading'>Συγχαρητήρια!</h4>\n"
            "<p style='text-align:center media='all''>Σύμφωνα με την φαρμακογονιδιωματική ανάλυση που πραγματοποιήθηκε και τον προβλεπόμενο φαινότυπο για κάθε γονίδιο που εξετάστηκε, δεν υπάρχουν αλλαγές στην δοσολογία φαρμάκων που θα πρέπει να προσέξετε.</p>\n"
            "<hr>\n"
            "<p style='text-align:center media='all'' class='mb-0'>Disclaimer: Συμβουλευτείτε τον γιατρό σας.</p>\n"
            "</div>\n"
        )
...
for sample in samples["sample_name"]:
    ...
    string = (
        ...
        "       <!-- Font Aweasome -->\n"
        "       <link rel='stylesheet' href='https://use.fontawesome.com/releases/v5.14.0/css/all.css' integrity='sha384-HzLeBuhoNPvSl5KYnjx0BT+WB0QEEqLprO+NBkkk5gbc67FTaL7XIGa2w1L0Xbgc' crossorigin='anonymous' media='all'>\n"
        "       <!-- Core theme CSS (includes Bootstrap)-->\n"
        f"      <style type='text/css' media='all'>{css_string}</style>\n"
        f"      <style type='text/css' media='all'>{css_heading_string}</style>\n"
        f"      <style type='text/css' media='all'>{css_body_string}</style>\n"       
        "       <!-- Place this tag in your head or just before your close body tag. -->\n"
        "       <script async defer src='https://buttons.github.io/buttons.js'></script>\n"
        "       <link rel='stylesheet' href='https://unpkg.com/bootstrap-table@1.17.1/dist/bootstrap-table.min.css' media='all'>\n"
        "   </head>\n"
        "<body id='page-top'>\n"
        ...
        "               <p style='text-align:justify media='all'' class='pre-wrap lead'>Thank you for choosing PGxSnake! PGxSnake is a bioinformatiscs tool for predicting how a person's DNA affects their response to hundreds of medications. PGxSnake does this by accurately calling star alleles (haplotypes), using Stargazer, in pharmacogenetic (PGx) genes, which are defined by single-nucleotide variants (SNVs), small insertion-deletions (indels), and/or large structural variants (SVs). Once identified, these star alleles can be translated to an activity score (AS), which is in turn used to predict the person's drug response.</p>\n"
        "            </div>\n"
        "            <div class='col-lg-5 mr-auto'>\n"
        "               <p style='text-align:justify media='all'' class='pre-wrap lead'>This report includes gene/drug pairs (e.g. CYP2D6/codeine) with accompanying levels of evidence for changing drug choice and dosing decisions. These pairs are described by the Clinical Pharmacogenetics Implementation Consortium (CPIC), which is an international assoication whose primary goal is to facilitate the use of PGx tests for patient care. Most importantly, CPIC provides detailed guidelines for helping clinicians understand how available genetic test results should be used to optimize drug therapy.</p>\n"
        "            </div>\n"
        "        </div>\n"
        "    </div>\n"
        "</section>\n"
